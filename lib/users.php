...
function send_reset_email($email, $name, $token, $isInvite) {
  ...
  $link = $scheme . '://' . $host . admin_url('set-password.php') . '?token=' . urlencode($token);
  $subject = $isInvite ? (SITE_NAME_ADMIN . ' — set up your admin account')
                       : (SITE_NAME_ADMIN . ' — reset your password');
  $intro = $isInvite
  ...
  $html = '<div style="font-family:sans-serif;max-width:520px">'
    . '<h2 style="color:#1a56db">' . htmlspecialchars(SITE_NAME_ADMIN) . ' Admin</h2>'
    . '<p>Hi ' . htmlspecialchars($name ?: '') . ',</p>'
    . '<p>' . $intro . '</p>'
    . '<p><a href="' . htmlspecialchars($link) . '" style="display:inline-block;background:#1a56db;color:#fff;'
  ...
  return send_admin_email($email, $name ?: $email, $subject, $html, $intro . "\n\n" . $link);
}
