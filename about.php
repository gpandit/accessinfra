...
const GALLERY_COLORS = ['#1d4ed8','#1e3a8a','#0c1f3f','#1a56db','#93c5fd','#0a2a6b'];
...
function IndiaLogoBadge() {
  return (
    <div role="img" aria-label="Access Infra Consulting logo over a map of India"
      ...
      <div style={{ position:'absolute', top:16, right:16, display:'flex', flexDirection:'column', alignItems:'center', gap:6, background:'#fff', padding:'14px 20px', borderRadius:14, boxShadow:'var(--shadow-lg)' }}>
        <img src={LOGO_URL} alt="Access Infra" style={{ width:'clamp(110px,12vw,160px)', height:'auto', display:'block' }} />
    ...
    <nav className="ai-nav" style={{ position:'sticky', top:0, left:0, right:0, zIndex:100, background:'var(--surface)', borderBottom:'1px solid var(--border)', boxShadow:scrolled?'var(--shadow)':'none', backdropFilter:'blur(12px)', transition:'box-shadow 0.3s', padding:'0 clamp(16px,5vw,80px)' }}>
      ...
      {open && (
        <div style={{ background:'var(--surface)', borderTop:'1px solid var(--border)', padding:'12px 20px 20px' }}>
          ...
          <a href={CONTACT} onClick={()=>setOpen(false)} style={{ display:'block', marginTop:14, textAlign:'center', background:'linear-gradient(135deg,#1a56db,#1e3a8a)', color:'#fff', textDecoration:'none', padding:'10px', borderRadius:8, fontSize:14, fontWeight:600 }}>{t.contactCta}</a>
        </div>
      )}
    </nav>
  );
        ...
        <div className="two-col" style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:40, alignItems:'center' }}>
          <div style={{ maxWidth:720 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, marginBottom:24, background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.25)', borderRadius:999, padding:'5px 14px 5px 10px' }}>
              <span style={{ width:6, height:6, borderRadius:'50%', background:'#93c5fd', display:'inline-block', boxShadow:'0 0 0 3px rgba(147,197,253,0.25)' }}></span>
            ...
            <h1 style={{ fontSize:'clamp(32px,5.5vw,68px)', fontWeight:800, color:'#fff', marginBottom:24, letterSpacing:'-1.5px', textWrap:'pretty', lineHeight:1.08 }}>
              {t.heading1}<br />
              <span style={{ color:'#93c5fd' }}>{t.headingGrad}</span><br />
              {t.heading2}
            </h1>
            ...
            <div style={{ display:'flex', gap:14, flexWrap:'wrap' }}>
              <a href="#services" style={{ background:'#fff', color:'#1e3a8a', textDecoration:'none', padding:'13px 26px', borderRadius:10, fontSize:14, fontWeight:700, boxShadow:'0 4px 20px rgba(12,31,63,0.3)', transition:'transform 0.2s' }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          {ov.features.map(f => (
            <div key={f} style={{ background:'var(--bg3)', borderRadius:10, padding:'12px 14px', display:'flex', alignItems:'center', gap:8, fontSize:13, fontWeight:500, color:'var(--text2)', border:'1px solid var(--border)' }}>
              <span style={{ color:'#1e3a8a', flexShrink:0 }}>✓</span> {f}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))', gap:16 }}>
        {t.tech.items.map(c => (
          <div key={c.num} style={{ background:'var(--bg3)', borderRadius:14, padding:20, border:'1px solid var(--border)' }}>
            <div style={{ width:30, height:30, borderRadius:7, background:'linear-gradient(135deg,#1a56db,#1e3a8a)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:14, marginBottom:10 }}>{c.num}</div>
            ...
            <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:5 }}>
              {c.specs.map(s => (
                <li key={s} style={{ fontSize:12.5, color:'var(--text2)', display:'flex', alignItems:'flex-start', gap:6 }}>
                  <span style={{ color:'#1e3a8a', marginTop:2, flexShrink:0 }}>✓</span>{s}
                </li>
              ))}
            </ul>
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))', gap:12 }}>
          {p.items.map(item => (
            <div key={item.n} style={{ display:'flex', gap:12, background:'var(--bg3)', borderRadius:12, padding:16, border:'1px solid var(--border)' }}>
              <div style={{ width:26, height:26, borderRadius:6, background:'linear-gradient(135deg,#1a56db,#1e3a8a)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, flexShrink:0 }}>{item.n}</div>
              <div>
  ...
  function Impact() {
    ...
    return (
      <div>
        ...
        <div style={{ background:'linear-gradient(135deg,#1a56db11,#1e3a8a11)', border:'1px solid #1a56db22', borderRadius:14, padding:24 }}>
          <h4 style={{ fontSize:17, fontWeight:700, marginBottom:12 }}>{imp.citizenHeading}</h4>
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {imp.words.map(w => (
              <span key={w} style={{ background:'linear-gradient(135deg,#1a56db,#1e3a8a)', color:'#fff', borderRadius:999, padding:'4px 12px', fontSize:12.5, fontWeight:600 }}>{w}</span>
            ))}
          </div>
        </div>
      </div>
          ...
          {t.items.map((c,i) => (
            <Reveal key={i} delay={i*80}>
              <div style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:18, overflow:'hidden', boxShadow:'var(--shadow)', transition:'transform 0.25s,box-shadow 0.25s' }}
                ...
                <div style={{ height:6, background:i===0?'linear-gradient(90deg,#1a56db,#1e3a8a)':'linear-gradient(90deg,#1e3a8a,#1a56db)' }}></div>
          ...
          {t.items.map((item,i) => (
            <Reveal key={i} delay={i*70}>
              <div style={{ background:'rgba(255,255,255,0.06)', backdropFilter:'blur(10px)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:16, padding:26 }}>
                <div style={{ color:'#93c5fd', fontSize:28, marginBottom:10, lineHeight:1 }}>"</div>
