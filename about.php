...
const GALLERY_COLORS = ['#1a4fba','#0f766e','#042f2e','#14b8a6','#0d7d73','#143060'];
...
function Tag({ children, color='blue', light=false }) {
  const bg  = color==='teal'?(light?'rgba(15,118,110,0.2)':'rgba(15,118,110,0.1)'):(light?'rgba(20,184,166,0.2)':'rgba(20,184,166,0.1)');
  const col = color==='teal'?'#0f766e':(light?'#93c5fd':'#14b8a6');
    ...
    <nav className="ai-nav" style={{ position:'sticky', top:0, left:0, right:0, zIndex:100, background:'var(--surface)', borderBottom:'1px solid var(--border)', boxShadow:scrolled?'var(--shadow)':'none', backdropFilter:'blur(12px)', transition:'box-shadow 0.3s', padding:'0 clamp(16px,5vw,80px)' }}>
      <div style={{ maxWidth:1280, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', height:68 }}>
        ...
        <div className="desktop-nav" style={{ display:'flex', alignItems:'center', gap:2 }}>
          ...
          <a href={HOME} style={{ color:'var(--text2)', textDecoration:'none', fontSize:13, fontWeight:500, padding:'6px 10px', borderRadius:6, transition:'all 0.15s' }} onMouseEnter={e=>{ e.target.style.color='#14b8a6'; e.target.style.background='var(--bg2)'; }} onMouseLeave={e=>{ e.target.style.color='var(--text2)'; e.target.style.background='transparent'; }}>Vendor Consulting</a>
          <a href={GOVT} style={{ color:'var(--text2)', textDecoration:'none', fontSize:13, fontWeight:500, padding:'6px 10px', borderRadius:6, transition:'all 0.15s' }} onMouseEnter={e=>{ e.target.style.color='#14b8a6'; e.target.style.background='var(--bg2)'; }} onMouseLeave={e=>{ e.target.style.color='var(--text2)'; e.target.style.background='transparent'; }}>Govt. Departments</a>
        </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <div style={{ display:'flex', gap:4 }}>
                {[{code:'en',label:'EN'},{code:'kn',label:'ಕನ್ನಡ'},{code:'hi',label:'हि'}].map(l => (
                  <button key={l.code} onClick={()=>setLang(l.code)} style={{ padding:'5px 10px', borderRadius:6, border:'1.5px solid', borderColor:lang===l.code?'#14b8a6':'var(--border)', background:lang===l.code?'rgba(20,184,166,0.1)':'var(--bg2)', color:lang===l.code?'#14b8a6':'var(--text2)', fontSize:12.5, fontWeight:600, cursor:'pointer' }}>{l.label}</button>
                ))}
              </div>
            </div>
            <FontSizeBtn fsIdx={fsIdx} cycleFontSize={cycleFontSize} />
          </div>
          <a href={CONTACT} onClick={()=>setOpen(false)} style={{ display:'block', marginTop:14, textAlign:'center', background:'linear-gradient(135deg,#14b8a6,#0f766e)', color:'#fff', textDecoration:'none', padding:'10px', borderRadius:8, fontSize:14, fontWeight:600 }}>{t.contactCta}</a>
        </div>
      )}
    </nav>
  );
...
function Hero({ lang }) {
  const t = T[lang].hero;
  return (
    <section id="top" style={{ minHeight:'100vh', display:'flex', flexDirection:'column', justifyContent:'center', padding:'clamp(100px,12vw,140px) clamp(16px,5vw,80px) 80px', background:'linear-gradient(135deg,#042f2e 0%,#0f766e 55%,#14b8a6 100%)', position:'relative', overflow:'hidden' }}>
        ...
        <div className="two-col" style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:40, alignItems:'center' }}>
          <div style={{ maxWidth:720 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, marginBottom:24, background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.25)', borderRadius:999, padding:'5px 14px 5px 10px' }}>
              <span style={{ width:6, height:6, borderRadius:'50%', background:'#6ee7b7', display:'inline-block', boxShadow:'0 0 0 3px rgba(110,231,183,0.25)' }}></span>
            ...
            <h1 style={{ fontSize:'clamp(32px,5.5vw,68px)', fontWeight:800, color:'#fff', marginBottom:24, letterSpacing:'-1.5px', textWrap:'pretty', lineHeight:1.08 }}>
              {t.heading1}<br />
              <span style={{ color:'#a7f3d0' }}>{t.headingGrad}</span><br />
              {t.heading2}
            </h1>
            ...
            <div style={{ display:'flex', gap:14, flexWrap:'wrap' }}>
              <a href="#services" style={{ background:'#fff', color:'#0f766e', textDecoration:'none', padding:'13px 26px', borderRadius:10, fontSize:14, fontWeight:700, boxShadow:'0 4px 20px rgba(4,47,46,0.3)', transition:'transform 0.2s' }}
  ...
  return (
    <section id="about" style={{ padding:'clamp(60px,8vw,120px) clamp(16px,5vw,80px)', background:'var(--bg2)' }}>
      <div className="two-col" style={{ maxWidth:1280, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'clamp(32px,6vw,80px)', alignItems:'center' }}>
        <Reveal>
          <div>
            ...
            <div style={{ display:'flex', gap:32, flexWrap:'wrap' }}>
              {t.stats.map(([n,l]) => (
                <div key={l}>
                  <div style={{ fontSize:24, fontWeight:800, fontFamily:'Sora,sans-serif', color:'#14b8a6' }}>{n}</div>
        ...
        <Reveal delay={120}>
          <ContextImage label="Access Infra team meeting with government department officials" gradient="linear-gradient(135deg,#14b8a622,#0f766e22)" />
          ...
          {t.items.map((s,i) => (
            <Reveal key={i} delay={(i%3)*60}>
              <div style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:16, padding:26, transition:'all 0.25s', cursor:'default' }}
                onMouseEnter={e=>{ e.currentTarget.style.transform='translateY(-4px)'; e.currentTarget.style.boxShadow='var(--shadow-lg)'; e.currentTarget.style.borderColor='#14b8a644'; }}
    ...
    return (
      <div className="two-col" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:40, alignItems:'start' }}>
        <div>
          ...
          <blockquote style={{ borderLeft:'3px solid #14b8a6', paddingLeft:16, fontStyle:'italic', color:'var(--text3)', fontSize:13.5, lineHeight:1.7 }}>{ov.quote}</blockquote>
          <div style={{ marginTop:18 }}>
            <a href={PDF_URL} target="_blank" rel="noopener noreferrer" style={{ display:'inline-flex', alignItems:'center', gap:8, color:'#14b8a6', textDecoration:'none', fontSize:13, fontWeight:600 }}>📄 Download DEEP Program Report →</a>
          </div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          {ov.features.map(f => (
            <div key={f} style={{ background:'var(--bg3)', borderRadius:10, padding:'12px 14px', display:'flex', alignItems:'center', gap:8, fontSize:13, fontWeight:500, color:'var(--text2)', border:'1px solid var(--border)' }}>
              <span style={{ color:'#0f766e', flexShrink:0 }}>✓</span> {f}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))', gap:16 }}>
        {t.tech.items.map(c => (
          <div key={c.num} style={{ background:'var(--bg3)', borderRadius:14, padding:20, border:'1px solid var(--border)' }}>
            <div style={{ width:30, height:30, borderRadius:7, background:'linear-gradient(135deg,#14b8a6,#0f766e)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:14, marginBottom:10 }}>{c.num}</div>
            ...
            <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:5 }}>
              {c.specs.map(s => (
                <li key={s} style={{ fontSize:12.5, color:'var(--text2)', display:'flex', alignItems:'flex-start', gap:6 }}>
                  <span style={{ color:'#0f766e', marginTop:2, flexShrink:0 }}>✓</span>{s}
                </li>
              ))}
            </ul>
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))', gap:12 }}>
          {p.items.map(item => (
            <div key={item.n} style={{ display:'flex', gap:12, background:'var(--bg3)', borderRadius:12, padding:16, border:'1px solid var(--border)' }}>
              <div style={{ width:26, height:26, borderRadius:6, background:'linear-gradient(135deg,#14b8a6,#0f766e)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, flexShrink:0 }}>{item.n}</div>
              <div>
  ...
  function Impact() {
    ...
    return (
      <div>
        ...
        <div style={{ background:'linear-gradient(135deg,#14b8a611,#0f766e11)', border:'1px solid #14b8a622', borderRadius:14, padding:24 }}>
          <h4 style={{ fontSize:17, fontWeight:700, marginBottom:12 }}>{imp.citizenHeading}</h4>
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {imp.words.map(w => (
              <span key={w} style={{ background:'linear-gradient(135deg,#14b8a6,#0f766e)', color:'#fff', borderRadius:999, padding:'4px 12px', fontSize:12.5, fontWeight:600 }}>{w}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <Reveal>
          ...
          <div style={{ display:'flex', gap:2, marginBottom:28, borderBottom:'1px solid var(--border)', overflowX:'auto' }}>
            {t.tabs.map((tab_,i) => (
              <button key={i} onClick={()=>setTab(i)} style={{ background:'none', border:'none', cursor:'pointer', padding:'10px 18px', fontSize:13.5, fontWeight:600, color:tab===i?'#14b8a6':'var(--text3)', borderBottom:tab===i?'2px solid #14b8a6':'2px solid transparent', marginBottom:-1, transition:'color 0.2s', whiteSpace:'nowrap' }}>{tab_}</button>
            ))}
          </div>
          {panels[tab]}
        </Reveal>
          ...
          {t.items.map((c,i) => (
            <Reveal key={i} delay={i*80}>
              <div style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:18, overflow:'hidden', boxShadow:'var(--shadow)', transition:'transform 0.25s,box-shadow 0.25s' }}
                ...
                <div style={{ height:6, background:i===0?'linear-gradient(90deg,#14b8a6,#0f766e)':'linear-gradient(90deg,#0f766e,#14b8a6)' }}></div>
                <div style={{ padding:26 }}>
                  <span style={{ fontSize:11.5, fontWeight:600, color:'#14b8a6', textTransform:'uppercase', letterSpacing:'0.06em' }}>{c.tag}</span>
                  ...
                  <div style={{ display:'flex', gap:20, marginBottom:22, paddingBottom:18, borderBottom:'1px solid var(--border)' }}>
                    {[c.m1,c.m2,c.m3].map(m => (
                      <div key={m.label}>
                        <div style={{ fontFamily:'Sora,sans-serif', fontSize:20, fontWeight:800, color:'#14b8a6' }}>{m.val}</div>
                        <div style={{ fontSize:11.5, color:'var(--text3)' }}>{m.label}</div>
                      </div>
                    ))}
                  </div>
                  <a href={c.href==='#contact'?CONTACT:c.href} style={{ color:'#14b8a6', textDecoration:'none', fontWeight:600, fontSize:13.5, display:'flex', alignItems:'center', gap:6, transition:'gap 0.2s' }}
...
function Testimonials({ lang }) {
  ...
  return (
    <section id="testimonials" style={{ padding:'clamp(60px,8vw,120px) clamp(16px,5vw,80px)', background:'var(--navy)', position:'relative', overflow:'hidden' }}>
      <div style={{ position:'absolute', top:'-30%', right:'-10%', width:500, height:500, borderRadius:'50%', background:'radial-gradient(circle,rgba(20,184,166,0.15),transparent 70%)' }}></div>
    ...
    <footer style={{ background:'var(--bg2)', borderTop:'1px solid var(--border)', padding:'clamp(28px,4vw,52px) clamp(16px,5vw,80px)' }}>
      <div style={{ maxWidth:1280, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:18 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:32, height:32, borderRadius:7, background:'linear-gradient(135deg,#14b8a6,#0f766e)', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ display:'flex', gap:20, flexWrap:'wrap' }}>
          {NAV_LINKS.map(l => (
            <a key={l.label} href={l.href} style={{ color:'var(--text3)', textDecoration:'none', fontSize:12.5, transition:'color 0.2s' }}
              onMouseEnter={e=>e.target.style.color='#14b8a6'} onMouseLeave={e=>e.target.style.color='var(--text3)'}>{l.label}</a>
          ))}
        </div>
